...
const MAC_APP_DIR = path.join(__dirname, 'app');
const MAC_WWWROOT = path.join(MAC_APP_DIR, 'wwwroot');
...
removeDirRecursive(MAC_APP_DIR);
fs.mkdirSync(MAC_WWWROOT, { recursive: true });

console.log('\n[3/3] Copying published assets to KannadaNudiMac/app/wwwroot...');
if (fs.existsSync(WEB_PUBLISH_WWWROOT)) {
  copyRecursive(WEB_PUBLISH_WWWROOT, MAC_WWWROOT);
...
const essentialFiles = [
  path.join(MAC_WWWROOT, 'index.html'),
  path.join(MAC_WWWROOT, '_framework', 'blazor.webassembly.js'),
  path.join(MAC_WWWROOT, 'lib', 'bootstrap', 'bootstrap.min.css'),
  path.join(MAC_WWWROOT, 'lib', 'bootstrap', 'bootstrap.bundle.min.js'),
